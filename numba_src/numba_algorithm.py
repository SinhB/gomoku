...
def numba_minimax(state, alpha, beta, depth, is_maximiser):
    ...
    if is_maximiser:
        ...
        for move in state.get_best_moves(is_maximiser):
            evaluation = numba_minimax(state.next_dc(move, -state.color), alpha, beta, depth - 1, False)
        ...
        for move in state.get_best_moves(not is_maximiser):
            evaluation = numba_minimax(state.next_dc(move, -state.color), alpha, beta, depth - 1, True)
